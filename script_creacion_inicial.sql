...
PAGO_CANT_CUOTAS
from gd_esquema.Maestra
WHERE PAGO_NRO_TARJETA IS NOT NULL AND PAGO_IMPORTE IS NOT NULL
...
JOIN SSGT.Publicacion pu on pu.id_vendedor = (SELECT tv.id_vendedor from SSGT.Vendedor tv where tv.d_cuit = m.VENDEDOR_CUIT and tv.d_razon_social = m.VENDEDOR_RAZON_SOCIAL) and
							pu.id_producto = (SELECT TOP 1 pr.id_producto from SSGT.PRODUCTO pr WHERE pr.d_descripcion = m.PRODUCTO_DESCRIPCION and
											pr.codigo_producto = m.PRODUCTO_CODIGO and
											pr.precio = m.PRODUCTO_PRECIO and
											pr.id_marca = (SELECT TOP 1 ma.id_marca from SSGT.Marca ma where ma.d_marca = m.PRODUCTO_MARCA) and
											pr.id_subrubro = (SELECT TOP 1 sub.id_subrubro from SSGT.Subrubro sub where sub.d_subrubro= m.PRODUCTO_SUB_RUBRO) and
											Pr.id_modelo = m.PRODUCTO_MOD_CODIGO)
